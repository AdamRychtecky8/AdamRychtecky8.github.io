# adamrychtecky8.github.io

Personal portfolio site for **Adam Rychtecky** — UCSB B.S. Statistics & Data Science + B.S. Psychological & Brain Sciences, now in an M.S. Data Science program.

**Live:** https://adamrychtecky8.github.io

> **This README doubles as a grounding document.** It's written so an AI agent (or a future me) can read this one file and know exactly what every feature does, where it lives, and how to change it — without re-reading the whole codebase. If you say *"make the blog tags bigger"* or *"add a project card"*, the agent should be able to find the answer here.
...
Hand-written **static HTML/CSS/JS**. No framework, no bundler, no npm, no Gemfile, **no build step**. A [.nojekyll](.nojekyll) file at the root tells GitHub Pages to skip Jekyll and serve files verbatim. Pushing to `main` publishes the repo root as-is within about a minute. Everything you edit is the thing that ships.
...
css/animations.css    fade-up keyframes, hero entrance, .project-card.hidden
css/blog.css          Blog-only styling: topic hues, log entries, post typography (~410 lines)

js/background.js      Canvas particle + noise background
js/nav.js             Active nav-link highlighting
js/blog.js            Blog manifest, routing, filtering, read-time, reading progress
...
There is **no global `.hidden` class**. Each feature defines its own: `.project-card.hidden` in [css/animations.css](css/animations.css), `.log-entry.hidden` in [css/blog.css:169](css/blog.css#L169). If you add a new filterable thing, you must add its own `.hidden { display: none }` rule.
...
| Section | What it is |
|---|---|
| Hero | Name, uppercase teal discipline line, italic mission line, then two buttons: `.btn-primary` → Projects, `.btn-secondary` → About. Text is inline-styled, not class-driven. |
...
`.about-header` puts `.profile-photo-box` (`assets/profile-photo.jpg`) beside a `.bio` with a "Background" label and two paragraphs. Then three `.work-card`s under "Where I'm Headed" (Systems of Intelligence · AI as Collaborator · Hard Problems, Done Properly), an italic centered closing line about reading, and the contact block. `.about-divider` rules separate the sections. Static; **no footer on this page**.
...
**Current cards:** Court-Vision · When Human and AI Crowds Agree Too Much · Modeling Perceptual Decisions: Humans vs. LLMs · Dialed · Wavelength · PM2.5 & Alzheimer's Review · Olympics (Gold, Growth, and Glory) · IMDb Revenue · plus one `[PROJECT NAME]` placeholder card. Several links are unfilled — see [§10](#10-known-issues-placeholders--fragile-spots).

### 5.4 [skills.html](skills.html) — Skills

The most structurally complex page. Four sections:
...
**d) Currently Building Toward.** `.building-strip` of three `.building-item`s, each a `.building-label` + `.building-sublabel`.
...
1. **Embedded PDF** — an `<iframe src="assets/Handshake-Resume.pdf" height="900px">` in an inline-styled card. Browser-dependent: some mobile browsers won't render a PDF in an iframe, which is why the download box below matters.
2. **Download box** — `.resume-download-box` with a `.btn-primary[download]` pointing at the same PDF.
3. **Education timeline** — `.timeline` of `.timeline-item`s (`.timeline-date` + `.timeline-body` with `<h3>`, `.org`, `<ul>`). UW M.S. Data Science (2026–2028) and UCSB B.S. ×2 (2022–2026, links `assets/FinalTranscript - AdamRychtecky.pdf`).
4. **Experience timeline** — same markup. UFCW Trust IT Infrastructure intern (2026) · VIU Lab Executive Research Assistant (2024–2026) · UFCW Trust IT intern (2024).

Note there are **four resume/transcript PDFs** in [assets/](assets/); only `Handshake-Resume.pdf` and `FinalTranscript - AdamRychtecky.pdf` are actually linked. `Graduate_School_resume.pdf` and `AR25_Transcript.pdf` are unreferenced.
...
Post **bodies** are plain Markdown in [blog/posts/](blog/posts/) with **no frontmatter** — `marked` renders the file as-is. Sorting is newest-first by `date`.
...
### Add a project card
Copy an existing `.project-card` block in [projects.html](projects.html). Set `data-category` to one or more of `ml psychology data other` (space-separated). Fill title, optional subtitle line, description, `.tag` spans, and `.card-link` anchors. Replace the `[PROJECT NAME]` placeholder card rather than adding a ninth if it's still there.
...
### Swap the resume PDF
Drop the new file in [assets/](assets/) and update **both** references in [resume.html](resume.html) — the `<iframe src>` and the download `<a href>`. Same filename = zero edits.
...
**Deploy.** [.github/workflows/deploy.yml](.github/workflows/deploy.yml) runs on every push to `main` (or manual dispatch): checkout → `configure-pages` → `upload-pages-artifact` with `path: '.'` (**the entire repo root, uncompiled**) → `deploy-pages`. Requires Settings → Pages → Source = **GitHub Actions**. Concurrency group `pages`, no cancel-in-progress. Live in roughly a minute. There is nothing to run locally before pushing.
...
- [projects.html](projects.html) — a full `[PROJECT NAME]` / `[ONE LINE DESCRIPTION]` placeholder card is still rendering, with `href="#"` links.
- [resume.html](resume.html) — UW timeline entry still shows `[DESCRIPTION — add once program details are set]` and `[TRANSCRIPT LINK — add once available]`.

### Content inconsistency
- **The graduate program is named two different ways.** [index.html](index.html) ("UW — MS Data Science"), [about.html](about.html) ("MS in Data Science program at the University of Washington"), and [resume.html](resume.html) ("University of Washington, 2026–2028") all say UW — but [skills.html](skills.html) "Currently Building Toward" says **"USC MS Applied Data Science — starting Fall 2026"**. One of these is stale.
...
### Asset hygiene
- **PDF naming is inconsistent:** `Handshake-Resume.pdf`, `Graduate_School_resume.pdf`, `AR25_Transcript.pdf`, `FinalTranscript - AdamRychtecky.pdf` (space in the filename, URL-encoded as `%20` in the link), `PSTAT100_finalproject.pdf`, `PSTAT126_FinalReport.pdf`, `ParticulateMatterReview.pdf`, `court-vision-report.pdf`.
- **Unreferenced assets:** `Graduate_School_resume.pdf`, `AR25_Transcript.pdf`, and `IMG_5238.PNG` (**~9 MB**, not used by any page — the largest file in the repo).

---

## 11. Dead scaffolding — do not edit

This repo began as GitHub's *"Skills: GitHub Pages"* course template (Jekyll + Minimal Mistakes) and was later rewritten as hand-rolled HTML. **The Jekyll scaffolding was never deleted and does nothing** — `.nojekyll` plus the deploy workflow mean none of it is ever built, and nothing on the live site links to it:

- [_config.yml](_config.yml) (pins `mmistakes/minimal-mistakes@4.24.0`; also still contains a placeholder `mailto:your@email.com` that never matched the real address), [_data/navigation.yml](_data/navigation.yml), [_posts/](_posts/), [_projects/](_projects/)
- [index.md](index.md), [about.md](about.md), [projects.md](projects.md), [coursework.md](coursework.md) — `about.md` even links a `/assets/transcript.pdf` that doesn't exist
...
**Editing these files will not change the live site.** They're safe removal candidates if you ever tidy up — confirm before deleting.

[docs/](docs/) is different: it's **not** a Pages source folder and not dead. It's hand-written per-page UI documentation kept for reference. Note it predates the blog, so it documents five pages, not six.
